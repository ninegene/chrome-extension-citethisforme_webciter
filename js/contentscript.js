chrome.runtime.onMessage.addListener(function(message) {
	if (message.type === "csToggleFrame"){
		var ctfmWebCiterFrame = document.querySelector('.ctfmWebCiterFrame');
		...
		if (ctfmWebCiterFrame) {
			...
			ctfmWebCiterFrame.setAttribute('class', 'ctfmWebCiterFrame');
			...
			ctfmWebCiterFrame.style.width = '322px';
...
function sendHighlightedText() {
    if (window.getSelection) {
		...
		console.log('CTFM highlighted text sent: ' + a);
    }
}
